...
BlockList LevelInitialize(char* filename) {
	...
	while (fgets(line, sizeof(line), file)) {
		...
		// при совпадении строк выдает 0
		if (!strcmp(line, "LevelStart")) {

			while (fgets(line, sizeof(line), file)) {
				line[strcspn(line, "\n")] = 0;

				if (!strcmp(line, "LevelEnd")) {
				...
				if (!strcmp(line, "BlockBegin")) {
					...
					while (fgets(line, sizeof(line), file)) {
						...
						if (!strcmp(line, "BlockEnd")) {

							block_list.block_list[block_list.count] = block;
							block_list.count++;
							break;
						}
						char* context = NULL;
						...
						if (!strcmp(key, "Size")) {
							...
							char* pos_context = NULL;
							int w, h;
							w = atoi(strtok_s(value, comma, &pos_context));
							h = atoi(strtok_s(NULL, none, &pos_context));
